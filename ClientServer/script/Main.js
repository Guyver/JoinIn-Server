...
var kinectData;
var kinectStatus;
var gameStatus = "Waiting for users."
var jsonObject;
 // Connection established.
 
kinect.onopen = function () 
{
    kinectStatus = "Connection successful.";
};

// Connection closed.
kinect.onclose = function () 
{
     kinectStatus = "Connection closed.";
};
		...
		{	
			...
			{				
				...
				{									
					...
					if( g_levelFourPlayed )
					{
						if( g_level == 4 )updateLevelFour( );					
					}
					else
					{					
...
function updateLevelThree( fps )
{
	...
	if( g_reachedMazeGoal && !g_waiting )
	{
		console.log( "Calling endLevel" );
		socket.emit( 'endLevel' );
		g_waiting = true;
...
function render()
{
	...
	guiCtx.fillText( 'Game Status: '+ gameStatus, 10, 300 );
	
	// Display the kinect Status.
	guiCtx.fillText( 'Kinect Status: '+ kinectStatus, 10, 350 );
...
{	
	...
	switch( key ){
		...
		case 65:// A
	  		// Rotate Left
			level_Manager.getPlayer().rotateModelLeftOnKey();
	  		break;
		case 68:// D
	  		// Rotate Right			
			level_Manager.getPlayer().rotateModelRightOnKey();
	  		break;
		case 87:// W
	  		// Rotate up
			// Move Forward
